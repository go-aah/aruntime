	...
	for _, s := range sections {
		...
		for _, ln := range s[1:] {
			...
			if lnCnt%2 == 0 { // File Path
				...
				// Strip base path `runtime.debug.strip_base_path = true`
				if st.StripSrcBase {
				...
				if !gr.HasPanic {
					gr.HasPanic = strings.HasPrefix(ln, panicPrefix)
...
func (st *Stacktrace) Print(w io.Writer) {
	...
	for _, gr := range st.GoRoutines {
		fmt.Fprint(w, "\n"+gr.Header+"\n")
		printFmt := "    %-" + strconv.Itoa(gr.MaxFuncLen+1) + "s-> %-" + strconv.Itoa(gr.MaxPkgLen) + "s #%s\n"
		for idx, f := range gr.Functions[gr.PanicIndex:] {
			fmt.Fprintf(w, printFmt, f, gr.Packages[idx], gr.LineNo[idx])
		}
	}
}
